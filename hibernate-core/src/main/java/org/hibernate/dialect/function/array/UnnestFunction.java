...
public class UnnestFunction extends AbstractSqmSelfRenderingSetReturningFunctionDescriptor {
	...
	public UnnestFunction(String defaultBasicArrayColumnName) {
	...
	public void render(
		...
		if ( ddlTypeCode == SqlTypes.JSON_ARRAY ) {
			renderJsonTable( sqlAppender, array, pluralType, sqlTypedMapping, tupleType, walker );
		}
		else if ( ddlTypeCode == SqlTypes.XML_ARRAY ) {
			renderXmlTable( sqlAppender, array, pluralType, sqlTypedMapping, tupleType, walker );
		}
		else {
			renderUnnest( sqlAppender, array, pluralType, sqlTypedMapping, tupleType, walker );
		}
	}
